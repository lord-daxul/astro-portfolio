...
* **WordPress (`https://backend.rauldavid.com`):** The single source of truth for editorial content (CPT `portfolio` / `projects`, `experiences`, and Blog `posts`).
* **WPGraphQL (`https://backend.rauldavid.com/graphql`):** The sole API gateway for querying structured WordPress data into the frontend.
* **Astro Frontend (`https://rauldavid.com`):** The presentation layer. Renders fast, accessible, semantic HTML using server rendering (`output: 'server'`) with `@astrojs/cloudflare`. Zero client-side JavaScript by default unless strictly required for interaction.
* **Cloudflare Workers:** The production edge runtime (`@astrojs/cloudflare` + `wrangler`) serving `https://rauldavid.com`.
* **Contact API:** Serverless endpoint `/api/contact` powered by Resend for email delivery.
* **GitHub:** Source code control, feature branch workflow, and CI/CD pipelines at `lord-daxul/astro-portfolio`.
...
3. **Strict Domain Mapping:** External WPGraphQL responses must pass through `src/lib/wordpress/mappers.ts` to convert CMS types into clean application domain types (`Project`, `Experience`, `Post`).
4. **Theme Alignment:** Support native CPT `portfolio` (`PortfolioItem` / `PortfolioItems`) with taxonomies `portfolio_category` and `tech` from `daxultheme`, as well as standard posts.
5. **Never Invent Fields:** Never invent WordPress custom fields or GraphQL fields without verifying against the schema or domain models.
...
4. **Dark Mode by Default:** The application defaults to dark mode, respecting stored preference in `localStorage` or explicit toggle with high contrast and accessible focus rings (`:focus-visible`).
